...
Validation is required. Every plan must include (1) the exact command(s) to run (Python/R entrypoints), (2) the expected artifacts to check (files/tables/figures), and (3) 1–3 sanity checks (e.g., row counts, schema checks, spot-check a few records, confirm outputs change as intended). Do not include long terminal transcripts, full logs, or novice-oriented explanations unless explicitly requested. If evidence is useful, include only a short excerpt (≤10 lines) or a diff that directly proves success.
