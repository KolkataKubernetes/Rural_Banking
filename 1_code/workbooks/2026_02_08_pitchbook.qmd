...
fig4c_data <- purrr::pmap_dfr(range_grid, function(start_year, end_year, range_label, range_display) {
  ...
  states_summed <- top10_plus_wi(state_period_range, "dealsize_summed") |>
    transmute(label = state_name, metric = "Summed", mode = "States", value = dealsize_summed)

  states_avg <- top10_plus_wi(state_period_range, "dealsize_average") |>
    transmute(label = state_name, metric = "Average", mode = "States", value = dealsize_average)
...
range_display <- range_grid$range_display
...
frame_list <- purrr::map(range_labels, function(rng) {
  frame_data <- purrr::map(combo_levels, function(combo_i) {
    ...
    list(x = df_i$label, y = df_i$value)
  })
...
pb <- plotly_build(
  p |>
    layout(
      title = "Figure 4C. Interactive Venture Deal Size",
      xaxis = list(
        title = "",
        tickangle = -30,
        type = "category",
...
pb$x$layout$sliders <- list(
  list(
    active = 0,
    currentvalue = list(prefix = "Year window: "),
    y = -0.28,
    x = 0,
    xanchor = "left",
    len = 0.88,
    steps = purrr::map2(range_labels, range_display, function(rng, rng_display) {
      list(
        method = "animate",
        args = list(
          list(rng),
