---
title: "02/10/2026 Update: Updated Pitchbook Figures"
...
fig3b_data <- purrr::map_dfr(year_min:year_max, function(y) {
  ...
  wi_rank_all <- state_year_single |>
    filter(!is.na(dealcount_per_million)) |>
    arrange(desc(dealcount_per_million)) |>
...
fig1_state <- state_period |>
  ...
  filter(!state_name %in% c("Delaware")) |>
...
ggplot(fig1_map, aes(long, lat, group = group, fill = committed_per_million)) +
  ...
  labs(
    ...
    subtitle = "2015-2024 totals; CA, DE, MA, and NY excluded",
    fill = "USD (millions)",
...
#ggsave(
#  filename = "/Users/indermajumdar/Documents/Research/Rural Banking/2025_WI_report/report/2026_02_28/visuals/fig1.png",
#  width = 8,
#  height = 6,
#  dpi = 300
#)
...
```{r fig1_wi_value_rank, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
fig1_rank_tbl <- fig1_state |>
  filter(!is.na(committed_per_million)) |>
  arrange(desc(committed_per_million)) |>
  mutate(rank = row_number())

wi_fig1 <- fig1_rank_tbl |>
  filter(state_name == "Wisconsin")

if (nrow(wi_fig1) == 0) {
  cat("Wisconsin is not present in the Figure 1 state set.")
} else {
  cat(
    paste0(
      "**Wisconsin Figure 1 value:** ",
      scales::comma(wi_fig1$committed_per_million[[1]], accuracy = 0.01),
      " USD per 1M residents  \n",
      "**Wisconsin rank:** ",
      wi_fig1$rank[[1]],
      " of ",
      nrow(fig1_rank_tbl),
...
fig2_plotly <- plot_ly(
  ...
  hovertemplate = paste0(
    ...
    "Year: %{x}<br>",
    "Committed per 1M: %{y:,.2f}<extra></extra>"
  )
) |>
  layout(
    title = list(text = "Figure 2. Capital Committed per 1M Residents Over Time<br><sup>2015-2024</sup>"),
    xaxis = list(
      title = "Year",
      tickmode = "array",
    ...
    yaxis = list(
      title = "USD (millions) per 1M residents",
      tickformat = ","
    ),
    hovermode = "x unified",
    ...
    annotations = list(
      list(
        text = "Source: Pitchbook Venture Monitor and BLS participation-based population estimates.",
        xref = "paper", yref = "paper",
        x = 0, y = -0.26,
        showarrow = FALSE,
        xanchor = "left",
        font = list(size = 11, color = "#5f5f5f")
      )
    )
  )
...
```{r fig2_noncapita, echo=FALSE, warning=FALSE, message=FALSE}
fig2_plotly <- plot_ly(
  fig2_data,
  x = ~year,
  y = ~numerator,
  ...
  hovertemplate = paste0(
    "Series: %{fullData.name}<br>",
    "Year: %{x}<br>",
    "Capital committed: %{y:,.2f}<extra></extra>"
  )
) |>
  layout(
    title = list(text = "Figure 2. Capital Committed Over Time<br><sup>2015-2024</sup>"),
    xaxis = list(
      title = "Year",
      tickmode = "array",
      tickvals = year_min:year_max,
...
wi_label_fig3 <- fig3_static |>
  filter(state_name == "Wisconsin") |>
  mutate(rank_label = paste0("(", wi_rank_fig3,")"))
...
ggplot(fig3_static, aes(x = dealcount_per_million, y = reorder(state_name, dealcount_per_million))) +
  ...
  geom_text(
    ...
    color = "#c5050c",
    hjust = 0,
    nudge_x = max(fig3_static$dealcount_per_million, na.rm = TRUE) * 0.03,
    fontface = "bold"
  ) +
  ...
  labs(
    ...
    y = NULL,
    caption = "Source: Pitchbook Venture Monitor and BLS participation-based population estimates. WI rank in parentheses"
  ) +
  theme_im()

# ggsave(
#   filename = "/Users/indermajumdar/Documents/Research/Rural Banking/2025_WI_report/report/2026_02_28/visuals/fig3.png",
#   width = 8,
#   height = 6,
#   dpi = 300
# )
...
wi_rank_fig4a <- state_period |>
  filter(!is.na(dealsize_summed)) |>
  arrange(desc(dealsize_summed)) |>
  mutate(rank_all_states = row_number()) |>
  filter(state_name == "Wisconsin") |>
  pull(rank_all_states)

wi_label_fig4a <- fig4a_static |>
  filter(state_name == "Wisconsin") |>
  mutate(rank_label = paste0("(", wi_rank_fig4a,")"))
...
ggplot(fig4a_static, aes(x = reorder(state_name, dealsize_summed), y = dealsize_summed)) +
  ...
  labs(
    ...
    x = NULL,
    y = "USD (millions)",
    caption = "Deal size = sum(capital committed) / sum(deal count). WI rank in parentheses."
  ) +
  theme_im() +
...
wi_rank_fig4b <- state_period |>
  filter(!is.na(dealsize_average)) |>
  arrange(desc(dealsize_average)) |>
  mutate(rank_all_states = row_number()) |>
  filter(state_name == "Wisconsin") |>
  pull(rank_all_states)

wi_label_fig4b <- fig4b_static |>
  filter(state_name == "Wisconsin") |>
  mutate(rank_label = paste0("(", wi_rank_fig4b,")"))
...
ggplot(fig4b_static, aes(x = reorder(state_name, dealsize_average), y = dealsize_average)) +
  ...
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    title = "Figure 4. Venture Deal Size by State (Average)",
    subtitle = "Top 10 states + Wisconsin, 2015-2024",
    x = NULL,
    y = "USD (millions)",
    caption = "Deal size = mean over years of state-year capital/dealcount ratios. WI rank in parentheses."
  ) +
  theme_im() +
  theme(axis.text.x = element_text(angle = 35, hjust = 1))

 # ggsave(
 #   filename = "/Users/indermajumdar/Documents/Research/Rural Banking/2025_WI_report/report/2026_02_28/visuals/fig4.png",
 #   width = 8,
 #   height = 6,
 #   dpi = 300
 # )
